...
// ─── Configuration de Build Moggcord ─────────────────────────────────────────

function killMoggcord() {
    const releaseDir = join(__dirname, "release", "moggcord-dist");
...
function buildMoggcordFromDiscord(discordApp) {
    ...
    console.log("[moggcord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Moggcord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "moggcord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const moggcordPreload = join(__dirname, "moggcord-preload.js");
    if (existsSync(moggcordPreload)) {
        cpSync(moggcordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "moggcord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Moggcord" --set-version-string "FileDescription" "Moggcord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[moggcord] Build terminé -> ${outDir}`);
}
...
buildMoggcordFromDiscord(discord);

module.exports = {
    appId: "com.moggcord.app",
    productName: "Moggcord",
    copyright: "Copyright 2026 Moggcord",
