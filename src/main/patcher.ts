...
console.log("[Moggcord] Starting up...");
...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (isMainWindow && process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Moggcord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Moggcord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Moggcord");
    ...
    console.log("[Moggcord] Running in vanilla mode. Not loading Moggcord");
}

console.log("[Moggcord] Loading original Discord app.asar");
