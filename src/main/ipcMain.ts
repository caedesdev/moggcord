...
import * as ghostNative from "../moggcordplugins/ghostClient/native";
(async () => {
    try {
        await (ghostNative as any).init(null);
    } catch (e) {
        console.warn("[Moggcord] Ghost-server pre-start failed:", e);
    }
})();
...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "moggcord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "moggcord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "moggcord-wbs-"));
...
`;
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "moggcord-kb-"));
            ...
            if (!groqKey) {
                ...
                fetch("https://api.groq.com/openai/v1/chat/completions", {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{
                            role: "user",
                            content: 'Give a very short definition (one simple sentence) for the following word, explaining concretely what it is without naming its grammatical category. Answer in English. Word: "' + bestWord + '"'
                        }]
                    }),
                })
                .then(r => r.json())
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "Moggcord QuickCSS Editor",
...
const OFFICIAL_UPDATE_URL = "https://github.com/caedesdev/moggcord/releases/latest/download/Moggcord-Installer.exe";
...
ipcMain.handle(IpcEvents.MOGGCORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "MoggcordUpdate-Setup.exe");
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Moggcord-VBCable");
    ...
    try {
        ...
        console.error("[Moggcord] VBCable install failed:", err);
