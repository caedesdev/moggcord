...
/**
 * Initializes the Moggcord system tray icon.
 ...
 */
export function initMoggcordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Moggcord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "moggcord", "Uninstall Moggcord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Moggcord", "Uninstall Moggcord.exe"),
                            join(process.env.PROGRAMFILES || "", "Moggcord", "Uninstall Moggcord.exe"),
                        ];
            ...
            console.log("[Moggcord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Moggcord] Failed to create tray:", e);
        }
    });
}
