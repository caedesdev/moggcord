...
function createMoggcordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Moggcord",
            submenu: [
                {
                    label: "About Moggcord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Moggcord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // On ne patch plus le menu tray — Moggcord gère son propre tray dans patcher.ts
}
