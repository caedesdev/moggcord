...
import { platformName } from "@moggcordplugins/equicordHelper/utils";
...
async function generateDebugInfoMessage() {
    ...
    const info = {
        Moggcord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/caedesdev/moggcord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Moggcord DevBuild": !IS_STANDALONE,
...
export default definePlugin({
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                        <Paragraph>You are using an outdated version of Moggcord! Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                return Alerts.show({
                    ...
                    body: <div>
                        ...
                        <Paragraph className={Margins.top8}>
                            Please join the <Link href="https://discord.gg/moggcord">Moggcord Server</Link> for support,
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of Moggcord, which we do not provide support for!</Paragraph>

                        <Paragraph className={Margins.top8}>
                            We only provide support for <Link href="https://github.com/caedesdev/moggcord">official builds</Link>.
                            Either <Link href="https://github.com/caedesdev/moggcord/releases/latest">switch to an official build</Link> or figure your issue out yourself.
                        </Paragraph>
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Moggcord developers"
        const moggcordGroupIds = [
        ...
        const isMoggcordGroup = moggcordGroupIds.includes(userId);

        if (!isOriginalDev && !isMoggcordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Moggcord developers";
