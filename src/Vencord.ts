...
import "~plugins";
console.log("%c[Moggcord]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("moggcord-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "moggcord-core-updater-root";
    ...
    titleSpan.textContent = "🔔 Moggcord Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Moggcord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Moggcord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Moggcord.",
