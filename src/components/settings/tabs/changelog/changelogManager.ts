...
const MOGGCORD_RELEASES_REPO = "caedesdev/moggcord";
const MOGGCORD_REPO_URL = `https://github.com/${MOGGCORD_RELEASES_REPO}`;
...
export { MOGGCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Moggcord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(MOGGCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
