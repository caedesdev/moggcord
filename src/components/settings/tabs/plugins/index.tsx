...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Moggcord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.MOGGCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("moggcord-tutorial-detected", handler);
        return () => window.removeEventListener("moggcord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.MOGGCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/moggcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/moggcordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { moggcordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const moggcordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Moggcord to function."
                ...
                const isMoggcord = folderName.startsWith("src/moggcordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isMoggcord) {
                    moggcordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { moggcordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...moggcordPlugins, ...othersPlugins];
    ...
    const moggcordVisible = moggcordPlugins.slice(0, Math.min(visibleCount, moggcordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - moggcordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Moggcord Plugins", value: SearchStatus.MOGGCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {moggcordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...moggcordVisible, ...othersVisible].length
                                    ? [...moggcordVisible, ...othersVisible]
                ...
                <>
                    {moggcordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Moggcord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {moggcordVisible}
                            </div>
                        </>
                    ...
                    {moggcordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
