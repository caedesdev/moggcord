...
export default function PluginModal({ plugin, onRestartNeeded, onClose, transitionState }: PluginModalProps) {
    ...
    function renderSettings() {
        ...
        if (!hasSettings || !settings)
            return <Paragraph>No settings available for this plugin.</Paragraph>;
            ...
            <ModalFooter>
                <Flex flexDirection="column" style={{ width: "100%" }}>
                    <Flex style={{ justifyContent: "space-between", alignItems: "center" }}>
                        {hasSettings ? (
                            <Tooltip text="Reset default settings" shouldShow={!isObjectEmpty(pluginSettings)}>
...
export function openWarningModal(plugin?: Plugin | null, onRestartNeeded?: (pluginName: string) => void, isPlugin = true, enabledPlugins?: number | null, reset?: () => void) {
    openModal(props => (
        <ConfirmModal
            ...
            header={isPlugin ? "Reset" : "Disable plugins"}
