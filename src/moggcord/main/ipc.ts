...
handleSync(IpcEvents.GET_GIT_HASH, () => Moggcord_GIT_HASH);
...
// Handler pour VencordNative.moggcord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.MoggcordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { MoggcordDir } = State.store;
    if (!MoggcordDir) return;

    const stats = await stat(MoggcordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(MoggcordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.MoggcordDir;
        return "ok";
    }
