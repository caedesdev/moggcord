...
import { t, useTranslation } from "../autoTranslateMoggcord";
import "./styles.css";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("moggcord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("moggcord_logs", JSON.stringify(toSave));
    } catch { }
}
...
const CFG: Record<LogType, { label: string; color: string; }> = {
    ...
    friend_request_cancel: { label: t("Canceled"), color: "#747f8d" },
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Moggcord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `moggcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
function subscribeToEvents() {
    ...
    sub("GUILD_MEMBER_UPDATE", d => {
        ...
        if (d.communicationDisabledUntil) {
            pushLog({ type: "guild_timeout", content: t("Temporarily timed out"), ...b, guildId: d.guildId, guildName: g?.name });
        }
    });
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("moggcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("moggcord-event-logs");
