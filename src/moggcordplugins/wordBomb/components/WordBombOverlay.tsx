...
const DICT_URLS = [
    ...
    "https://raw.githubusercontent.com/moggcordoff/dicofr/refs/heads/main/dico.txt"
];
...
export function WordBombOverlay() {
    ...
    const sendWord = async (word: string) => {
        ...
        if (safeMode) {
            ...
            if (!groqKey) {
                setDefinition("Error: Groq API key missing in MoggcordAI.");
            } else {
                fetch("https://api.groq.com/openai/v1/chat/completions", {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{
                            role: "user",
                            content: `Give a very short definition (one simple sentence) for the following word, explaining concretely what it is without naming its grammatical category. Answer in English. Word: "${word}"`
                        }]
                    }),
                })
                .then(r => r.json())
