...
import { groqChat, getGroqKey } from "../moggcordAI/groqManager";
            ...
            <div style={{
                ...
                <div>
                    ...
                    <div style={{ fontSize: "13px", marginTop: "4px" }}>
                        ...
                        Please configure it once in the <strong>MoggcordAI</strong> settings.
                    </div>
                </div>
            </div>
        )
        ...
        if (!apiKey) {
            try {
                ...
                openConfirmationModal({
                    ...
                    content: "AutoResponder requires a Groq API Key to function. Please configure it once in the MoggcordAI settings.",
                    confirmText: "Configure MoggcordAI",
                    cancelText: "Cancel",
                    onConfirm: () => {
                        const { openModal } = findByPropsLazy("openModal");
                        // Logique pour ouvrir les settings MoggcordAI si possible
                    }
                });
            } catch (e) {
...
1. SHORT REPLIES: Keep replies concise (one or two sentences max). Do not write long paragraphs.
2. DISCREET PERSONAL INFO: Only use my personal info when relevant. Do not force it into every message.
3. NATURAL WRITTEN STYLE: In text, do not say "uh" or "wait" while calculating. Give the result or continue the sentence. Remove oral hesitation.
4. HUMAN TONE: Speak like a friend on Discord. Light texting style is allowed if it fits my style.
