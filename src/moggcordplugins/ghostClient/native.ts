 ...
 * Lance ghost-server via node.exe bundlé dans moggcord-dist
...
// Handler IPC pour ouvrir les URLs externes (utilisé par MoggcordUpdater)
ipcMain.handle("MOGGCORD_OPEN_URL", (_event, url: string) => {
...
async function api(endpoint: string, body?: object, timeoutMs = 15000): Promise<any> {
    ...
    if (!ok) return { ok: false, error: "ghost-server not found or timed out" };
