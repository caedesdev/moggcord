...
const DS_KEY_TOKENS = "moggcord-ghost-tokens";
const DS_KEY_AUTO_FOLLOW = "moggcord-ghost-autofollow";
const DS_KEY_MIC_DEVICE = "moggcord-ghost-mic-device-label";
const DS_KEY_SELECTED = "moggcord-ghost-selected";
        ...
        Native.listAudioInputDevices().catch(() => []).then(async (devs: any[]) => {
            ...
            if (names.length) {
                ...
                if (savedMic === null || savedMic === undefined) {
                    ...
                    console.log("[GhostClient] Loading the user's preferred microphone:", savedMic);
                }
            }
        }).catch(() => { });
    }, []);
...
export default definePlugin({
    ...
    async start() {
        ...
        try {
            ...
            console.warn("[GhostClient] ipc.on unavailable:", e?.message);
        }
    },
