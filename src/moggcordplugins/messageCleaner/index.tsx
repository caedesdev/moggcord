...
async function cleanChannel(channelId: string) {
    ...
    try {
        ...
        log(`🧹 Starting cleanup for "${channelName}"`);
        ...
        while (!shouldStopCleaning) {
            try {
                ...
                for (const message of validMessages) {
                    if (shouldStopCleaning) { log("Stop requested by the user"); break; }
                ...
                if (statusCode === 429) {
                    log("Rate limit reached, pausing for 30s...", "warn");
        ...
        log(`✅ Cleanup complete: ${deleted} deleted, ${failed} failed, ${skipped} skipped — ${timeStr}`);
    ...
    if (isCleaningInProgress) {
        ...
        menuItems.push(
            <Menu.MenuItem key="cleaning-status" id="vc-cleaning-status"
                label={`Cleaning: ${percentage}% (${processed}/${total})`}
                color="brand" disabled={true} />,
            <Menu.MenuItem key="stop-cleaning" id="vc-stop-cleaning"
                label="Stop cleanup" color="danger" action={stopCleaning} />
        );
    } else {
        menuItems.push(
