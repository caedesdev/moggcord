/*
 * Moggcord — MultiInstance native.ts
 *
...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "moggcord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Moggcord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__moggcord_token', { value: TOKEN, writable: false });
        ...
        console.log("[MoggcordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[MoggcordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `moggcord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:moggcord-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `Moggcord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Moggcord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[MoggcordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__MOGGCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Moggcord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.moggcord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[MoggcordMI] arrangeSplit error:", e);
    }
}
