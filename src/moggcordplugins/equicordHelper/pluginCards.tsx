...
import { EQUIBOT_USER_ID, MOGGCORD_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Moggcord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components — Moggcord Bot (moggcord.online, Component v2 Container format)
    if (message.author.id === MOGGCORD_BOT_USER_ID) {
        ...
        if (containerComponents?.length >= 3) {
            ...
            if (pluginUrl?.startsWith("https://moggcord.online/plugins/")) {
