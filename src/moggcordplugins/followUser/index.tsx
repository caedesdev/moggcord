...
function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(() => {
        if (followedId) {
            Toasts.show({ message: `Follow inactive for 30 minutes. Stopped following ${followedName}`, type: Toasts.Type.FAILURE, id: Toasts.genId() });
            unfollow();
        }
    }, INACTIVITY_MS);
}
    ...
    for (const s of states) {
        ...
        if (newCh !== followedChannel) {
            ...
            if (newCh) {
                ...
                joinChannel(newCh);
                Toasts.show({ message: `Following ${followedName} → voice`, type: Toasts.Type.MESSAGE, id: Toasts.genId() });
            }
        }
    }
}
...
async function follow(userId: string) {
    ...
    if (followedChannel) {
        ...
        Toasts.show({ message: `Following ${name} ✓. Joined their voice channel`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
    } else {
        Toasts.show({ message: `Following ${name} ✓. Waiting for a voice channel`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
    }
}
...
async function unfollow() {
    ...
    await persist();
    notifyAll();
    if (name) Toasts.show({ message: `Stopped following ${name}`, type: Toasts.Type.MESSAGE, id: Toasts.genId() });
}
