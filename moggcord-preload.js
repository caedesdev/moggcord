// Moggcord preload — globalPaths fix + Equicord avec contextBridge
"use strict";
(function () {
    ...
    try {
        ...
        if (rendererJs) {
            webFrame.executeJavaScript(rendererJs).catch(e => {
                console.error("[Moggcord] renderer inject failed:", e?.message);
            });
        }
    } catch (e) {
        console.error("[Moggcord] VencordPreloadGetRendererJs failed:", e);
    }
