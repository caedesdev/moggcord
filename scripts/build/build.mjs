...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "moggcordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/moggcord/renderer.js",
        format: "iife",
        target: ["esnext"],
    ...
    },
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/preload.ts")],
        outfile: "dist/moggcord/preload.js",
        footer: { js: "//# sourceURL=file:///VencordPreload\n" + sourceMapFooter("preload") },
        sourcemap,
        define: {
            ...defines,
...
await Promise.all([
    ...
    writeFile("dist/moggcord/package.json", JSON.stringify({
        name: "moggcord",
        main: "main.js"
    }))
]);
...
if (!watch && !IS_DEV) {
    await Promise.all([
        ...
        obfuscateDir("dist/moggcord"),
    ]);
} else {
    console.log("[build] Obfuscation skipped in dev/watch mode.");
}
*/
console.log("[build] Obfuscation disabled (source left readable).");

await Promise.all([
    createPackage("dist/desktop", "dist/desktop.asar"),
    createPackage("dist/moggcord", "dist/moggcord.asar"),
]);
