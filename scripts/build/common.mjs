...
let gitHashValue;
try {
    gitHashValue = process.env.MOGGCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
} catch {
    gitHashValue = process.env.MOGGCORD_HASH || "unknown";
}
export const gitHash = gitHashValue;
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "moggcordplugins", "moggcordplugins/_api"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.MOGGCORD_REMOTE;
            if (!remote) {
                try {
...
export const commonOpts = {
    ...
    alias: {
        ...
        "@moggcordplugins": "./src/moggcordplugins",
    }
};
