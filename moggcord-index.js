...
// ── CRITIQUE : userData = dossier Moggcord pour les settings/plugins
...
const moggcordData = path.join(app.getPath("appData"), "Moggcord");
app.setPath("userData", moggcordData);
...
// pour le token Discord stable. Quand Moggcord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Moggcord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Moggcord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Moggcord] Module bloqué (inutile pour Moggcord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Moggcord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Moggcord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Moggcord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(moggcordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Moggcord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Moggcord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Moggcord] LevelDB check failed:", e.message); }

// Modules bundlés dans moggcord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Moggcord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Moggcord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans moggcord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Moggcord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Moggcord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Moggcord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Moggcord] Impossible de patcher build_info.json:", e.message);
}
