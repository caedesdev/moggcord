...
RC_OBJ = $(BUILD_DIR)/moggcord_rc.o
...
all: $(BUILD_DIR)/moggcord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/moggcord_rc.o: moggcord.rc assets/moggcord.ico
	mkdir -p $(BUILD_DIR)
	windres moggcord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/moggcord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/moggcord.desktop
	rm -f $(PREFIX)/share/pixmaps/moggcord.png
	rm -f $(PREFIX)/share/pixmaps/moggcord.svg
	rm -f $(PREFIX)/bin/moggcord

install: uninstall
	cp moggcord.desktop $(PREFIX)/share/applications/moggcord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/moggcord.svg
	cp $(BUILD_DIR)/moggcord $(PREFIX)/bin/moggcord

run: all
	cp assets/moggcord.gschema.xml $(BUILD_DIR)/assets/moggcord.gschema.xml
